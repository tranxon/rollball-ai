...
# 默认行为：required = true（至少一个平台支持才安装）

[[tools]]
name = "bluetooth"
type = "builtin"
platforms = { desktop = false, mobile = true, web = false }
...
# 不声明 required 默认为 true

[[tools]]
name = "image_filter"
type = "wasm"
binary = "./tools/image_filter.wasm"
platforms = { desktop = true, mobile = true, web = true }
# WASM 天然跨平台
```
...
1. **安装时检查**：Gateway 的 Package Manager 解析 manifest，检查当前平台是否满足所有 `required = true` 的 tools/skills。如果有 required tool 在当前平台不可用，安装时给出警告："此 Agent 的以下功能在当前平台不可用：shell（仅 desktop）。是否继续安装？"
2. **运行时降级**：对于 `required = false` 的 tools/skills，Agent Runtime 启动时根据当前平台过滤可用工具列表，不可用的 tool 不注入 system prompt，LLM 不会尝试调用它。
3. **Skill 级联降级**：如果 Skill 依赖的 tool 在当前平台不可用，该 Skill 自动降级为不可用，并在 Agent 的能力描述中标注。
4. **能力声明同步**：Agent 注册到 Gateway 的 capabilities 中，也需要反映平台差异。其他 Agent 通过 Intent 调用时，Gateway 可以在路由时检查目标 Agent 的当前平台能力。
...
✅ All core tools available on this platform (desktop)
⚠️  Optional skill "bluetooth-beacon" unavailable (mobile only)
...
| 跨平台兼容性 | required/optional + platforms 枚举 | 声明式、渐进式降级、Skill 级联自动处理 |
