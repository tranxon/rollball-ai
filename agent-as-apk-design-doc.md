...
设计一个去中心化、高安全、可扩展的 AI Agent 运行时平台。核心思想是将每个 Agent 视为一个独立的"应用包"（类似 Android APP），运行在客户端（用户电脑）并由轻量级 Gateway 管理生命周期。Agent 的工作流代码统一、可复制，差异仅在于本地配置和持久化记忆（Memory）。Memory 作为独立后端服务，提供跨设备同步和向量检索能力。
...
| 特性 | Agent as APP | ZeroClaw / OpenClaw | Docker + 微服务 |
|------|-------------|---------------------|----------------|
