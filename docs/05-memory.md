...
> 本文档基于 docs/review/04-p2-s2-design-review.md S2 设计评审更新。主要变更：memory_store 接口从三元组改为自然语言（§4.1）、Purge 三条路径（§5.2）、检索降级策略（§6.1）、graph_expand 自适应深度（§6.3）、新增冲突处理章节（§6.4）、Token 分层计数与弹性预算分配（§1）。
