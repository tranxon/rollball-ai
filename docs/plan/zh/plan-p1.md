...
| 阶段 | 名称 | 核心目标 |
|------|------|---------|
| **S1** | 基础层 | workspace 骨架 + acowork-core + acowork-sign + acowork-vault |
...
acowork-core (S1)
    ...
    ├── acowork-vault (S1) ←┘
    │
    ├── acowork-runtime (S2) ←─── acowork-sign + acowork-vault
    │
    └── acowork-gateway (S3) ←─── acowork-core + acowork-sign + acowork-vault
                                      │
                                      └── acowork-runtime (S2)
```
...
**说明**：7 个 crate 为 `acowork-core`、`acowork-memory`、`acowork-runtime`、`acowork-gateway`、`acowork-grafeo`、`acowork-vault`、`acowork-sign`。Phase 1 暂不深入 `acowork-memory` 和 `acowork-grafeo`（Phase 2），但目录结构先建立。

---

### S1.2 任务：acowork-core
...
### S1.3 任务：acowork-sign

**目标**：实现 `.agent` 包签名机制，提供 `acowork-keygen`、`acowork-sign`、`acowork-verify` 三个 CLI 命令。
...
- CLI：`acowork-keygen --type developer --output-dir <path>` / `--type platform`
...
- 输出签名后的 .agent 包
- CLI：`acowork-sign --input <pkg.agent> --key <keyfile> --output <signed.agent>`
...
- 校验各 section 摘要完整性
- 验证签名（Ed25519）
- CLI：`acowork-verify <pkg.agent>` → 输出 signer 信息或错误
...
**验收标准**：`acowork-sign sign + acowork-verify verify` 往返成功；签名前后 ZIP 内容一致（Signing Block 插入后不影响已有文件）；CLI 帮助信息完整。

---

### S1.4 任务：acowork-vault
...
| S3.2.4 签名验证委托（调用 `acowork-sign` 验签） | 未签名/无效包拒绝加载 |

**说明**：包加载器不内联签名逻辑，而是调用 `acowork-sign` crate 的 API。
...
**目标**：`acowork-gateway` 二进制，支持守护进程模式和 CLI 命令。

| 任务 | 验收标准 |
|------|---------|
| S4.1.1 CLI 入口（clap）：`--daemon`, `install`, `uninstall`, `start`, `stop`, `list` 子命令 | `--help` 输出正确 |
| S4.1.2 守护进程模式（后台常驻） | `acowork-gateway --daemon` 常驻 |
...
| 5.1.2 用 `acowork-sign` 签名 | 签名成功 |
| 5.1.3 Gateway CLI 安装：`acowork-gateway install weather-agent.agent` | 安装成功 |
| 5.1.4 启动天气 Agent：`acowork-gateway start com.example.weather` | Runtime 进程启动 |
| 5.1.5 发送对话消息，验证 LLM 调用 + 工具调用 + 响应返回 | 完整往返成功 |
| 5.1.6 停止 Agent：`acowork-gateway stop com.example.weather` | 进程终止 |
...
**说明**：需要真实 LLM API 的测试用环境变量跳过（`ACOWORK_TEST_SKIP_LIVE_LLM=1` 跳过）。
...
| S1.2 | acowork-core 类型定义 | acowork-core | S1 | 完成 | 33 tests |
| S1.3 | acowork-sign 签名工具链 | acowork-sign | S1 | 完成 | 21 tests |
| S1.4 | acowork-vault 密钥存储 | acowork-vault | S1 | 完成 | 20 tests |
| S3.1 | Runtime CLI + 配置 | acowork-runtime | S2 | 完成 | |
| S3.2 | .agent 包加载器 | acowork-runtime | S2 | 完成 | |
| S3.3 | LLM Provider 实现 | acowork-runtime | S2 | 完成 | OpenAI + Ollama + Router + Reliable |
| S3.4 | Agent 主循环 | acowork-runtime | S2 | 完成 | 9步循环 + BudgetGuard + LoopDetector |
| S3.5 | 内置工具（13个） | acowork-runtime | S2 | 完成 | 合并http_get/http_post→http_request |
| S3.6 | History Manager | acowork-runtime | S2 | 完成 | FIFO裁剪+ToolResult折叠+PreemptiveTrim |
| S3.7 | IPC 客户端 | acowork-runtime | S2 | 完成 | UnixSocket + NamedPipe + GatewayClient |
| S4.1 | Gateway CLI | acowork-gateway | S3 | 完成 | clap子命令+daemon模式+配置加载 |
| S4.2 | IPC 服务端 | acowork-gateway | S3 | 完成 | UnixSocket+NamedPipe+Session+6种Handler |
| S4.3 | 包管理器 | acowork-gateway | S3 | 完成 | install+uninstall+upgrade |
| S4.4 | 生命周期管理器 | acowork-gateway | S3 | 完成 | process spawn/kill/health+idle timeout |
| S4.5 | Key Vault 集成 | acowork-gateway | S3 | 完成 | VaultFacade+KeyRelease分发 |
...
|----------|--------|----------|----------|----------|----------|
| #16 | P2 | IPC Handler 错误返回空字符串 | `acowork-gateway/src/ipc/server.rs` | ✅ 已修复 | Handler 错误返回改为 `error: Option<String>`，Runtime client 适配 4 种情况处理 |
| #17 | P2 | KeyReleaseResult 语义模糊 | `acowork-core/src/protocol.rs` | ✅ 已修复 | 改为 `{ api_key: Option<String>, error: Option<String> }`，可区分成功/Key不存在/Vault未解锁/其他错误 |
...
AgentCowork 开发中优先复用已验证的模式和代码，避免重复造轮子。

**复用原则**：
- 只复用"相对独立"的代码——完整模块、完整函数、边界清晰的 trait
- 需要大幅修改才能适配 AgentCowork 架构的代码，不直接复用
- 不复用与单进程模式深度耦合的部分（如 Runtime 内嵌的 HTTP Server）

**Phase 1 重点复用领域**：

| 领域 | 参考来源 | AgentCowork 落地位置 | 说明 |
|------|---------|-----------------|------|
| Tool trait | - | `acowork-core/src/tools/traits.rs` | 直接复用 trait 定义 |
| Provider trait | - | `acowork-core/src/providers/traits.rs` | 直接复用 trait 定义 |
| Schema 清洗 | - | `acowork-runtime/src/tools/schema.rs` | adaptation |
| Streaming 解析 | - | `acowork-runtime/src/providers/` | adaptation |
| Loop Detector | - | `acowork-runtime/src/agent/loop_detector.rs` | adaptation（三种模式 + 三级响应） |
| History Manager | - | `acowork-runtime/src/agent/history.rs` | adaptation（token 计算 + FIFO 裁剪） |
| RateLimitedTool | - | `acowork-runtime/src/tools/wrappers.rs` | adaptation |
| PathGuardedTool | - | `acowork-runtime/src/tools/wrappers.rs` | adaptation |
| JSON-RPC frame | - | `acowork-core/src/protocol.rs` | adaptation |
| Vault 加密 | - | `acowork-vault/src/encryption.rs` | 借鉴加密逻辑 |

**复用要求**：
- 注释标明来源：`// Adapted from <source>` 
- 显著偏离时注明原因：`// AgentCowork deviation: <reason>`
- 不直接依赖外部 crate，而是 adaptation 后复制到对应 crate

**不直接复用的部分**（需要重新设计）：
- 单一巨型配置文件 → AgentCowork 按 crate 拆分
- 单进程 Agent Loop → AgentCowork 需要 IPC 层
- 文件系统沙箱（bubblewrap 集成方式）→ AgentCowork Phase 3 重新实现
- 配置驱动机制 → AgentCowork 改用 manifest 声明驱动
...
- **跳过条件**：真实 API 调用在 CI 中跳过（`ACOWORK_TEST_SKIP_LIVE_LLM=1`）
