...
| -------- | ------ | ----------------------------------------------------- | -------------------------------------------- | -------- |
| #1       | P1     | AcoworkError 过于宽泛，Provider 错误缺少 status_code | `acowork-core/src/error.rs`                 | S1       |
| #2       | P0     | 签名块存储改为二进制级嵌入（APK v2 思路）             | `acowork-sign/src/sign.rs`                  | S1       |
| #3       | P1     | AgentLoop 不持有 GatewayClient，IPC 链路断裂          | `acowork-runtime/src/agent/loop_.rs`        | S1       |
| #4       | P1     | 主循环缺少流式处理集成                                | `acowork-runtime/src/agent/loop_.rs`        | S1       |
| #5       | P1     | BudgetGuard session_tokens 代替 daily_tokens          | `acowork-runtime/src/agent/budget_guard.rs` | S4       |
| #6       | P2     | Token 估算过于粗糙（4字符/token）                     | `acowork-runtime/src/agent/history.rs`      | S5       |
| #7       | P2     | GatewayState 无并发保护                               | `acowork-gateway/src/gateway/state.rs`      | S1       |
| #8       | P2     | IPC Server 同步阻塞（单连接）                         | `acowork-gateway/src/ipc/server.rs`         | S1       |
| #9       | P2     | Budget/Usage/Rate Handler 是占位符                    | `acowork-gateway/src/ipc/server.rs`         | S4       |
| #10      | P2     | Grafeo 全部 unimplemented                             | `acowork-grafeo/src/`                       | S2       |
...
| 任务                                                    | 涉及 crate                     | 验收标准                                                                                       |
| ------------------------------------------------------- | ------------------------------ | ---------------------------------------------------------------------------------------------- |
| S2.0.1 替换 Cargo.toml 依赖（rusqlite → grafeo-engine） | acowork-grafeo                | 删除 rusqlite，引入 grafeo-engine v0.5.39（crates.io）                                         |
...
| S2.0.3 验证编译通过                                     | acowork-grafeo                | `cargo check` 无错误；Workspace 依赖声明正确                                                   |
| S2.0.4 MemoryStore trait 适配 grafeo-engine API         | acowork-core, acowork-grafeo | trait 方法签名与 GrafeoDB API 对齐；GrafeoStore stub 实现编译通过                              |
| S2.0.5 可选子 trait 拆分                                | acowork-core                  | EpisodicStore, SemanticStore, ForgettingStore 等模块化 trait                                   |
...
| S2.4.4 向量 Embedding 存储             | acowork-grafeo                               | episode / knowledge 节点写入时，将 Runtime 层传入的 `Vec<f32>` 存入 `embedding` 属性                  |
| S2.4.5 索引自动恢复                    | acowork-grafeo                               | Agent 重启后 GrafeoDB 自动恢复 HNSW 索引；5000 条 episode 向量检索 P99 < 100ms                        |
...
| 任务                              | 文件                                     | 验收标准                     |
| --------------------------------- | ---------------------------------------- | ---------------------------- |
| S2.9.1 MemoryManager 结构实现     | `acowork-runtime/src/memory/manager.rs` | 生命周期阶段协调             |
...
| S2.9.5 Runtime 集成 MemoryManager | `acowork-runtime`                       | 主循环调用                   |
...
MemoryManager 包含业务逻辑（Token 裁剪策略、优先级排序等），归属 acowork-runtime。
acowork-memory 保持为瘦 wrapper，仅导出 MemoryStore trait 定义。
...
| S2.13.2 并发控制               | acowork-grafeo                              | 利用 GrafeoDB MVCC 快照隔离，多 Session 并发读写；无需自研锁                                        |
| S2.13.3 Embedding 降级链路     | acowork-runtime                             | Local → Remote → Disabled 三级（Runtime LLM Provider 职责，见 S5.3）                                |
| S2.13.4 Grafeo 故障处理        | acowork-grafeo                              | 健康检查（`MATCH (n) RETURN count(n)`）+ `grafeo-file` 单文件备份 + Grafeo WAL 自动恢复             |
...
| S2.15.2 Runtime 层集成         | `acowork-runtime/src/memory/manager.rs` | `MemoryQuery.min_score` 从 Agent manifest.toml `[memory.retrieval]` 配置读取，默认 0.6                               |
| S2.15.3 System Prompt 动态注入 | `acowork-runtime/src/prompt/builder.rs` | Abstention 触发时注入拒答指引（约 30 tokens）："当检索分数不足时回复'我不确定这个信息'，不要猜测"                    |
| S2.15.4 manifest 可配置        | `acowork-core/src/manifest.rs`          | 支持 `[memory.retrieval]` 表配置 `min_score = 0.6`，工具型 Agent 可降至 0.5，学习型可升至 0.7                        |
...
| S3.1.4 签名并安装到 Gateway          | `acowork-sign` 签名          |

#### S3.2 任务：身份信息系统

| 任务                                | 文件                            | 验收标准                            |
| ----------------------------------- | ------------------------------- | ----------------------------------- |
| S3.2.1 Identity Category 定义       | `acowork-core/src/identity.rs` | Identity/Preferences/Knowledge/Work |
| S3.2.2 PrivacyLevel 枚举            | `acowork-core/src/identity.rs` | Public/Personal/Sensitive           |
| S3.2.3 IdentityStore 接口           | `acowork-core/src/identity.rs` | store/query/observe                 |
| S3.2.4 System Agent Grafeo 存储身份 | `acowork-grafeo`               | 私有存储                            |
...
| S4.1.2a Agent 启动就绪判断                          | acowork-gateway       | IPC 握手完成（identity_delivery 回复）视为就绪；复用 S3.3 冷启动逻辑 |
...
| ----- | ---------------------------------------------------------- | --------------------------------- | ---- | -------------- | ---------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1.1  | AgentLoop 注入 GatewayClient                               | acowork-runtime                  | S1   | -              | 5          | ✅    | `ipc_client: Option<GatewayClient>` 已实现，CLI `connect_gateway_client()` 已集成                                                                                                                 |
| S1.2  | IPC Server 改异步多连接                                    | acowork-gateway                  | S1   | S1.1           | 5          | ✅    | `Arc<RwLock<GatewayState>>` + `tokio::spawn` per connection，02-p2-s1-code-review 确认                                                                                                            |
| S1.3  | 签名块二进制级嵌入                                         | acowork-sign                     | S1   | -              | 8          | ✅    | V2 binary format `to_binary()`/`from_binary()` + `zip_utils.rs`，向后兼容 V1                                                                                                                      |
| S1.4  | 结构化错误类型改进                                         | acowork-core                     | S1   | -              | 6          | ✅    | `ProviderError` 含 `status_code` + `ProviderErrorType` + `retryable`，`ReliableProvider` 改用结构化判断                                                                                           |
| S1.5  | 流式处理集成到主循环                                       | acowork-runtime                  | S1   | S1.1           | 8          | ✅    | `chat_stream()` + `StreamEvent` 状态机，02-p2-s1-code-review 确认                                                                                                                                 |
| S1.6  | AgentLoop InboundQueue                                     | acowork-runtime                  | S1   | S1.1           | 6          | ✅    | `mpsc::channel(64)` + `InboundMessage` 三类型 + drain 逻辑，背压测试通过                                                                                                                          |
| S1.7  | 工具调度改并行执行                                         | acowork-runtime                  | S1   | S1.1           | 5          | ✅    | `spawn+select+channel` 分层超时，`tool_timeout_ms`/`iteration_timeout_ms` 独立配置                                                                                                                |
| S2.0  | grafeo-engine 依赖集成与 MemoryStore Trait 适配            | acowork-core, acowork-grafeo    | S2   | S1             | 8          | ✅    | 依赖替换+features配置+编译验证完成                                                                                                                                                                |
| S2.1  | Grafeo LPG 数据模型实现                                    | acowork-grafeo                   | S2   | S2.0           | 16         | ⬚    |                                                                                                                                                                                                   |
| S2.2  | 经历层（Episodic）实现                                     | acowork-grafeo                   | S2   | S2.1           | 12         | ⬚    |                                                                                                                                                                                                   |
| S2.3  | 沉淀层（Semantic）实现                                     | acowork-grafeo                   | S2   | S2.1           | 15         | ⬚    |                                                                                                                                                                                                   |
| S2.4  | 向量索引（grafeo-engine HNSW）集成                         | acowork-grafeo                   | S2   | S2.1           | 6          | ⬚    |                                                                                                                                                                                                   |
| S2.5  | 全文索引（grafeo-engine BM25）集成                         | acowork-grafeo                   | S2   | S2.1           | 4          | ⬚    |                                                                                                                                                                                                   |
| S2.6  | 巩固管道（Consolidation）                                  | acowork-grafeo                   | S2   | S2.2,S2.3      | 14         | ⬚    |                                                                                                                                                                                                   |
| S2.7  | 遗忘衰减机制（Decay）                                      | acowork-grafeo                   | S2   | S2.3           | 8          | ⬚    |                                                                                                                                                                                                   |
| S2.8  | 关联扩散检索（GQL + PageRank + topology_boost + 动态权重） | acowork-grafeo                   | S2   | S2.3,S2.4,S2.5 | 14         | ⬚    |                                                                                                                                                                                                   |
| S2.9  | MemoryManager 集成                                         | acowork-runtime                  | S2   | S2.0~S2.8      | 12         | ⬚    |                                                                                                                                                                                                   |
| S2.10 | 冲突检测与处理（三层信号）                                 | acowork-grafeo                   | S2   | S2.6           | 10         | 🚧    | S2.10.1 基础结构已完成，S2.10.2~S2.10.5 待集成                                                                                                                                                    |
| S2.11 | 隐私访问控制                                               | acowork-gateway                  | S2   | S2.10          | 6          | ⬚    |                                                                                                                                                                                                   |
| S2.12 | 质量评估框架（RetrievalMetrics + LongMemEval + LLM Judge） | acowork-grafeo                   | S2   | S2.0~S2.11     | 10         | ⬚    |                                                                                                                                                                                                   |
| S2.13 | 工程约束与降级策略                                         | acowork-grafeo                   | S2   | S2.4           | 10         | ⬚    |                                                                                                                                                                                                   |
| S2.14 | 备份与恢复                                                 | acowork-grafeo                   | S2   | S2.0           | 3          | ⬚    |                                                                                                                                                                                                   |
| S2.15 | Abstention 阈值机制实现                                    | acowork-grafeo, acowork-runtime | S2   | S2.4,S2.5      | 4          | ⬚    |                                                                                                                                                                                                   |
| S3.1  | System Agent 包和清单                                      | examples/system-agent             | S3   | -              | 3          | ✅    | manifest `system=true` + prompts/ 齐全，capabilities 声明完整                                                                                                                                     |
| S3.2  | 身份信息系统                                               | acowork-core                     | S3   | -              | 6          | ✅    | `IdentityStore` trait + `GrafeoStore` 完整实现 store/query/observe/list_all                                                                                                                       |
| S3.3  | 冷启动身份注入                                             | acowork-gateway                  | S3   | S3.1,S3.2,S1.2 | 5          | ✅    | `auto_start_system_agent()` + `build_identity_delivery()` + Runtime `load_identity_delivery()`，03-p2-s2-code-review 确认 GTW-12 已实现                                                           |
| S3.4  | Identity 工具完整化                                        | acowork-runtime                  | S3   | S3.2           | 8          | 🚧    | 三个工具文件已创建（identity_query/store/observe），工具骨架+参数校验+测试完整，但 IPC 链路未贯通：工具未实际调用 GrafeoStore，identity_query/observe 返回 placeholder，identity_store 仅打印确认 |
| S4.1  | Intent 跨 Agent 转发                                       | acowork-gateway                  | S4   | S1.2           | 10         | ⬚    |                                                                                                                                                                                                   |
| S4.2  | Capability Registry                                        | acowork-gateway                  | S4   | S4.1           | 8          | ⬚    |                                                                                                                                                                                                   |
| S4.3  | Budget Tracker 完整实现                                    | acowork-gateway                  | S4   | S1.2           | 10         | ⬚    |                                                                                                                                                                                                   |
| S4.4  | Rate Limiter 完整实现                                      | acowork-gateway                  | S4   | S1.2           | 8          | ⬚    |                                                                                                                                                                                                   |
| S4.5  | UsageReport 上报链路贯通                                   | acowork-runtime                  | S4   | S1.1,S4.3      | 6          | ⬚    |                                                                                                                                                                                                   |
| S5.1  | Anthropic Provider                                         | acowork-runtime                  | S5   | -              | 10         | ⬚    |                                                                                                                                                                                                   |
| S5.2  | Provider 动态路由切换                                      | acowork-runtime                  | S5   | S5.1           | 6          | ⬚    |                                                                                                                                                                                                   |
| S5.3  | Embedding 生成（ONNX）                                     | acowork-grafeo                   | S5   | S2.4           | 8          | ⬚    |                                                                                                                                                                                                   |
| S5.4  | Token 计数精度改进                                         | acowork-runtime                  | S5   | -              | 6          | ⬚    |                                                                                                                                                                                                   |
