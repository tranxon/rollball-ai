...
**涉及 crate**：新增 `apps/acowork-desktop/`（Tauri 项目）、`acowork-runtime`（Session + Episode 提炼）、`acowork-gateway`（Session API + IPC 转发）、`acowork-memory`（record_distilled）、`acowork-grafeo`（Episode 写入）
...
**涉及 crate**：`acowork-runtime`（新增 `debug/` 模块）、`acowork-gateway`（新增 DevMode 启动参数）
...
编译验证代码：`acowork-runtime/tests/select_borrow_check_test.rs`（保留作为借用模式参考）
...
| 任务                                    | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 预期测试 | 验收标准                                                                       |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------ |
| S2.1 Debug Protocol Server              | 新增 `acowork-runtime/src/debug/server.rs`：WebSocket 服务端（`tokio-tungstenite`，`ws://127.0.0.1:19877`）；JSON-RPC 2.0 消息解析/响应；DevMode 检测（`--dev-mode` 启动时才监听）；连接管理（单客户端，后续连接拒绝）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 8        | WebSocket 连接建立/断开、JSON-RPC 消息收发                                     |
...
> **前置依赖**：S2.9 依赖 S1.10 的 Gateway 记忆 API 已就绪；依赖 Grafeo 引擎的调试接口（`grafeo::debug` 模块）；巩固和遗忘扫描调用 `acowork-grafeo` 内部 API。
...
>
> **涉及 crate**：`acowork-runtime`（扩展 `skills/` + `debug/`）
...
| S3.3 Grafeo Skill 经验层（核心数据结构） | 新增 `acowork-grafeo/src/skills/` 模块：`SkillDraft` / `SkillIteration` / `SkillExecution` / `SkillExperience` 节点类型；节点关系边（`HAS_ITERATION` / `EXECUTED_AS` / `PUBLISHED_AS` / `HAS_EXPERIENCE`）；CRUD 操作接口               | 8        | 经验层节点创建/查询/更新正确                   |
...
**涉及 crate**：`acowork-gateway`（新增克隆/发布 API）、`acowork-sign`（验证集成）
...
| S4.3.3 可选签名集成                           | `package_manager/publish.rs` | sign=true 时调用 `acowork_sign::sign::sign_package()`；sign=false 时仅打包不签名；签名使用 dev key 或指定 key_dir                                                                                                  |
...
`acowork-gateway package --source <dir> --output <dir> [--sign] [--key-dir <dir>]`
...
acowork package --source examples/system-agent --output build/

# 2. CLI 打包 weather-agent
acowork package --source examples/weather-agent --output build/

# 3. 通过 Gateway HTTP API 安装
curl -X POST http://127.0.0.1:19876/api/agents/install \
  -H 'Content-Type: application/json' \
  -d '{"package_path": "build/com.acowork.system-1.0.0.agent", "dev_mode": true}'
...
**涉及 crate**：`acowork-gateway`、`acowork-core`、`acowork-grafeo`、`acowork-runtime`
