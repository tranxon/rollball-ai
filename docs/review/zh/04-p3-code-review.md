...
| 任务 | 计划内容 | 实现文件 | 状态 | 备注 |
|------|---------|---------|------|------|
| S1.1 权限模型完善 | `Permission` 枚举 + `PermissionGrant` + `PermissionPolicy` | `acowork-core/src/permission.rs` | ✅ 完成 | 覆盖全部 7 类权限，序列化/反序列化完整 |
| S1.2 权限持久化存储 | Gateway 侧 `PermissionStore`（rusqlite） | `acowork-gateway/src/permission_store.rs` | ✅ 完成 | Schema 版本管理、CRUD、按 agent_id 查询 |
| S1.3 安装时权限审查 | 安装流程增加权限审查 | `acowork-gateway/src/package_manager/` | ⚠️ 部分 | 权限声明解析已实现，用户交互流程待 Desktop App |
| S1.4 运行时权限校验器 | Runtime 侧 `PermissionChecker` 缓存 | `acowork-runtime/src/tools/permission_checker.rs` | ✅ 完成 | O(1) 分类索引、策略自动批准、缓存失效 |
| S1.5 运行时权限请求 | Runtime → Gateway `PermissionRequest` | 协议层待实现 | ⚠️ 延期 | 依赖 IPC 消息路由（S4 阶段），框架已预留 |
| S1.6 权限升级通知 | Agent 升级检测权限差异 | `acowork-gateway/src/package_manager/` | ⚠️ 部分 | 权限对比逻辑已实现，用户确认流程待 Desktop |
| S1.7 权限撤销与重置 | CLI 命令 `acowork permission revoke/reset` | `PermissionStore::revoke/reset` | ✅ 完成 | Runtime 缓存失效机制完整 |
...
| 任务 | 计划内容 | 实现文件 | 状态 | 备注 |
|------|---------|---------|------|------|
| S2.1 Wasmtime 引擎集成 | `WasmEngine` 单例 + Fuel metering | `acowork-runtime/src/tools/wasm/engine.rs` | ✅ 完成 | Cranelift 配置、内存限制、Fuel 计量 |
| S2.2 WASM 实例管理 | `WasmToolInstance` 加载/执行/销毁 | `acowork-runtime/src/tools/wasm/instance.rs` | ✅ 完成 | 线性内存管理、超时终止、Host 函数注册 |
| S2.3 WASI Preview 2 权限映射 | manifest 权限 → WASI capability | `acowork-runtime/src/tools/wasm/wasi_mapper.rs` + `sandbox.rs` | ✅ 完成 | 目录预开放、网络能力隔离、`inherit_network()` |
| S2.4 WIT 组件模型升级 | `acowork-tool.wit` 接口定义 | `acowork-runtime/src/tools/wasm/wit.rs` + `component.rs` | ✅ 完成 | 类型安全绑定、向后兼容 Phase 1 |
| S2.5 acowork-tool-sdk | proc macro + 自动 JSON 序列化 | `acowork-tool-sdk/` | ✅ 完成 | `#[tool]` 宏、schema 导出、wasm32-wasip2 目标 |
| S2.6 WASM 工具集成测试 | 示例工具端到端测试 | `acowork-runtime/src/tools/wasm/` tests | ⚠️ 部分 | 引擎/沙箱单元测试完成，E2E 待示例工具编译 |
...
| 任务 | 计划内容 | 实现文件 | 状态 | 备注 |
|------|---------|---------|------|------|
| S3.1 FileProvenance 文件来源追踪 | `FileProvenanceStore`（rusqlite）+ 工作区扫描 | `acowork-runtime/src/security/file_provenance.rs` | ✅ 完成 | 4 种来源类型、持久化、智能路径解析 |
| S3.2 ShellRisk 风险分级引擎 | 四级分类 + 命令解析器 | `acowork-runtime/src/security/shell_risk.rs` | ✅ 完成 | 白名单/黑名单、sudo/eval/pipe 检测 |
| S3.3 命令-文件关联分析 | `assess_shell_risk()` 交叉查询 | `shell_risk.rs` L284-317 | ✅ 完成 | Downloaded/Unknown 提升到 High |
| S3.4 Approval Gate | trait + CLI 实现 | `acowork-runtime/src/security/approval_gate.rs` | ✅ 完成 | `ApprovalGate` trait、CLI/Desktop 预留 |
| S3.5 工作区文件系统监控 | `FsWatcher`（notify crate） | `acowork-runtime/src/security/fs_watcher.rs` | ✅ 完成 | 跨平台抽象、symlink 检测、可执行文件识别 |
| S3.6 审计日志 | JSON 结构化日志 | `acowork-runtime/src/security/audit_log.rs` | ✅ 完成 | 按日分割文件、完整字段记录 |
...
| 任务 | 计划内容 | 实现文件 | 状态 | 备注 |
|------|---------|---------|------|------|
| S4.1 巩固调度器 | `ConsolidationScheduler` 定时/idle/手动 | `acowork-grafeo/src/consolidation/scheduler.rs` | ✅ 完成 | 批次管理、进度跟踪、idle 检测 |
| S4.2 三元组提取 | LLM 驱动的三元组提取 + 去重 | `acowork-grafeo/src/consolidation/triple_extraction.rs` | ✅ 完成 | JSON 解析、subject+predicate 去重、写入沉淀层 |
| S4.3 冲突分类与证据验证 | Evolution/Correction/Ambiguous 分类 | `acowork-grafeo/src/conflict.rs` + `consolidation/conflict_llm.rs` | ✅ 完成 | 启发式分类 + LLM 仲裁 |
| S4.4 经验泛化（模式提炼） | ProceduralNode 生成 | `acowork-grafeo/src/consolidation/generalization.rs` | ✅ 完成 | 多 Episode 模式识别、置信度评估 |
| S4.5 质量评估框架 | `RetrievalMetrics` + LongMemEval | `acowork-grafeo/src/retrieval_metrics.rs` + `eval.rs` | ✅ 完成 | precision@k/recall@k/MRR、5 维评测 |
...
1. ⚠️ **权限字符串格式与 Android 不完全一致**：Android 使用 `com.example.permission.CAMERA` 风格，当前使用 `network:https://...` 风格
   - **评价**：当前格式更语义化，适合 AgentCowork 场景，但需在文档中明确说明差异
...
```rust
// acowork-core/src/permission.rs L137-158
...
```rust
// acowork-runtime/src/tools/wasm/sandbox.rs L92-148
...
```rust
// acowork-runtime/src/security/shell_risk.rs L284-317
