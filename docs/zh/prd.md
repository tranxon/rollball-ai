...
AgentCowork 是一个"**Agent as APP**"平台。核心隐喻借鉴 Android：Agent 如 APK 是声明式包，Agent Runtime 如 ART 是统一执行引擎，Gateway 如 AMS 管理生命周期。

**平台双定位——AgentCowork 同时服务于两类用户：**
...
> 企业级 Agent 不是 AgentCowork 平台内置的能力，而是 Agent 开发的一种范式——Agent 开发者通过标准 RAG 接口对接企业知识库，用户感知到的只是一个普通的 Agent。

**设计原则**：

- **纯对接，不托管**：AgentCowork 不运营 RAG 服务，知识属于企业自己；AgentCowork 定义标准查询协议（请求/响应 JSON Schema），企业 RAG 自行适配此协议，AgentCowork 不为各家 RAG 系统实现 adapter
...
| RAG-02 | RAG 工具支持标准查询接口（AgentCowork 定义请求/响应 JSON Schema，企业 RAG 自行适配协议） | P2 | 兼容所有符合标准协议的 RAG 系统——随 RAG-01 一起交付 |
...
企业 RAG 集成属于"企业级 Agent 开发范式"，不要求所有 Agent 都支持 RAG，也不出现在 AgentCowork 核心平台的功能承诺中。RAG 仅当 manifest 声明时使能，Runtime 行为由配置驱动。
...
> AgentCowork 不只是终端用户的 AI 工具，也是 Agent 开发者的创作平台。开发者用声明式包构建能力，无需编写可执行代码；平台提供从编写到发布的完整工具链。
...
| 优先级 | 含义 | 阶段 | 说明 |
|--------|------|------|------|
| P0 | 平台核心——没有就不叫 AgentCowork | Phase 1 | MVP 必须交付 |
...
某企业开发"销售助手 Agent"，manifest 声明 `[[tools]] type = "rag"`，指向企业内部的 Qdrant RAG 服务（含产品知识库、销售话术库、合规文档）。用户安装后在 Desktop App 与 Agent 对话，Agent 同时查询本地 Grafeo（记住该用户的偏好、历史提问）和企业 RAG（检索产品参数、竞品对比、合规要点），拼接后给出回答。RAG 服务由企业自己运维，AgentCowork 平台不接触任何企业数据，用户量增长对 AgentCowork 云端压力为零。
...
| 术语 | 定义 |
|------|------|
| Agent | AgentCowork 平台上的独立 AI 应用，以 .agent 包分发 |
...
| 企业 RAG | 企业自建的 RAG 知识库服务，Agent 通过标准 rag 工具接入，不经 AgentCowork 云端中转 |
| 双通道检索 | Agent 同时查询本地 Grafeo 和企业 RAG 两条通道的检索模式 |
| work Zone | Grafeo 沉淀层中与个人工作相关的记忆分区（原 enterprise Zone），与 AgentCowork 企业 RAG 无关 |
...
**上下文**：
从个人和小团队扩展到企业用户场景时，需要回答两个问题：（1）企业 RAG 与本地 Grafeo 是同一层还是不同通道？（2）AgentCowork 是否要自己托管 RAG 服务？

**决策**：

1. 采用双通道检索模型（Mode A）：本地 Grafeo 和企业 RAG 是两条独立的检索通道，结果拼接后送入 LLM 上下文。两者不整合进统一的记忆抽象层。
2. AgentCowork 不托管 RAG 服务（Option 1）：企业自建或采购 RAG 系统，AgentCowork 在 manifest 中声明 RAG 接入点（URL + 认证），运行时作为标准工具调用。AgentCowork 云端零状态。
3. 企业 RAG 定位为"企业级 Agent 开发范式"，不属于 AgentCowork 核心平台功能承诺。
...
记忆系统是 AgentCowork 的核心差异化能力（推理靠 LLM、操作靠 Tools，只有记忆是自主掌控的），必然经历大量迭代。当时存在三个紧耦合风险：
...
AgentCowork 的长期愿景包括支持基于硬件传感器的 Agent（如机器人、IoT 设备）。当前 Tool 系统基于 WASM 沙箱，天然隔离硬件访问，无法满足硬件传感器的适配需求。传感器访问的延迟需求横跨多个数量级（GPS 0.1Hz → IMU 1000Hz+），单一机制无法覆盖。
