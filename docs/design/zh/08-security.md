...
借鉴 Android 生态的 Google Play 扫描机制，AgentCowork 在 Agent 仓库（商店）上架阶段建立安全关卡，将安全问题**左移（Shift-Left）**到发布环节，形成"上架前扫描 + 安装时验签 + 运行时防护"的三层纵深：
...
Grafeo 记忆扫描是 AgentCowork 特有的挑战，在传统应用安全领域没有直接对应。核心问题：
